...
if (! function_exists('countWaitingPayment')) {
    function countWaitingPayment()
    {
        $currentUser = auth('customer')->user();
        $GET_PAYMENTS = Payment::where([['customer_id', $currentUser->id], ['status', 'pending']])->groupBy('charge_id')->get();

        return $GET_PAYMENTS->count();
    }
}