...
abstract class TableAbstract extends DataTable
{
    ...
    public int $pageLength = ($pageLength) ? $pageLength : 10;
    ...
    public function getColumns(): array
    {
        ...
        if ($this->tableLang && $this->repository) {
    ...
    public function htmlDrawCallback(): ?string
    {
        ...
        if ($this->collapse) {
            return 'function () {' . $this->htmlDrawCallbackFunction() . ' ' . $this->htmlDrawCallbackRowGroup() . '}';
        }

        return 'function () {' . $this->htmlDrawCallbackFunction() . '}';
    }

    public function htmlDrawCallbackRowGroup(): ?string
    {
        $url = $this->routeGroup;
        return "
            var api = this.api();
            ...
            // Mengelompokkan berdasarkan kolom pertama
            var rows = api.rows({ page: 'current' }).nodes();
            var lastGroup = null;

            api.column($this->colGroup, { page: 'current' }).data().each(function (group, i) {
                    $.ajax({
                        url: '$url?group=' + group,
                        method: 'GET',
                        dataType: 'html',
                        success: function(response) {
                            ...
                            var tableRow = $('<tr>').append($('<td>').attr('colspan', rows.columns().nodes().length).append(tableHTML));
                            $(rows).eq(i).after(tableRow);
                        },
