    ...
    <div class="bg-light py-md-5 px-lg-3 px-2">
        <div class="container-xxxl rounded-7 bg-white py-lg-5 py-md-4 py-3 px-3 px-md-4 px-lg-5">
            <div class="row">
                    ...
                    <div class="product-details js-product-content">
                        <div class="entry-product-header">
                            <div class="product-header-left">
                                <h1 class="fs-5 fw-normal product_title entry-title product-title-name">
                                    {!! BaseHelper::clean($product->title) !!}</h1>
                                <div class="product-entry-meta">
                                    <p class="css-vni7t6-unf-heading e1qvo2ff8">
                                        <span class="main">Terjual</span>
                                        {{ $product->original_product->terjual }}
                                    </p>
                                    @if ($product->brand_id)
                ...
                <div class="col-lg-3 col-md-4 ps-4 product-details js-product-content">
                    {{--  {!! dynamic_sidebar('product_detail_sidebar') !!}  --}}
                    {!! Theme::partial(
                        'ecommerce.product-cart-form',
                        compact('product', 'selectedAttrs') + [
                            'withButtons' => true,
                            'withVariations' => true,
                            'wishlistIds' => [],
                            'withBuyNow' => true,
        ...
        <div class="row product-detail-tabs mt-3 mb-4">
            <div class="col-md-3">
                <div class="nav flex-column nav-pills me-3" id="product-detail-tabs" role="tablist"
                    ...
                    <a class="nav-link active" id="product-description-tab" data-bs-toggle="pill"
                        href="#product-description" type="button" role="tab" aria-controls="product-description"
                        aria-selected="true">
                        {{ __('Description') }}
                    </a>
                    @if (EcommerceHelper::isReviewEnabled())
                        <a class="nav-link" id="product-reviews-tab" data-bs-toggle="pill" href="#product-reviews"
                            type="button" role="tab" aria-controls="product-reviews" aria-selected="false">
                            {{ __('Reviews') }} ({{ $product->reviews_count }})
                        </a>
                    @endif
                    @if (is_plugin_active('marketplace') && $product->store_id)
                        <a class="nav-link" id="product-vendor-info-tab" data-bs-toggle="pill"
                            href="#product-vendor-info" type="button" role="tab"
                            aria-controls="product-vendor-info" aria-selected="false">
                            {{ __('Vendor Info') }}
                        </a>
                    @endif
                    @if (is_plugin_active('faq') && count($product->faq_items) > 0)
                        <a class="nav-link" id="product-faqs-tab" data-bs-toggle="pill" href="#product-faqs"
                            type="button" role="tab" aria-controls="product-faqs" aria-selected="false">
            ...
            <div class="col-md-9">
                <div class="tab-content" id="product-detail-tabs-content">
                                            ...
                                            <div class="col-md-10">
                                                <div class="average-rating border py-4 px-4">
                                                    <h3 class="h1 average-value text-red">
                                                        {{ number_format($product->reviews_avg, 2) }}</h3>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-12 mb-3 d-flex mt-2">
                                                            <label class="required"
                                                                for="rating">{{ __('Your rating') }}:</label>
                                                        ...
                                                        <div class="col-12 mb-3">
                                                            <label class="required"
                                                                for="txt-comment">{{ __('Review') }}:</label>
                                                            <textarea class="form-control" name="comment" id="txt-comment" required aria-required="true" rows="8"
                                ...
                                @if ($product->reviews_count)
                                    <div class="product-reviews-container my-5">
                                        ...
                                        <product-reviews-component
                                            url="{{ route('public.ajax.product-reviews', $product->id) }}">
                                        </product-reviews-component>
                                    </div>
                                @endif
                            </div>
                    ...
                    @endif
                    @if (is_plugin_active('faq') && count($product->faq_items) > 0)
                        <div class="tab-pane fade" id="product-faqs" role="tabpanel"
    ...
    <div class="container-xxxl">
        <div class="row">
            <div class="col-12">
                ...
                <related-products-component limit="6"
            ...
            <div class="container">
                <article class="ps-product--header-sticky">
                    ...
                    <div class="ps-product__wrapper">
                        <div class="ps-product__content"><a class="ps-product__title product-title-name"
                                href="{{ $product->url }}">{!! BaseHelper::clean($product->title) !!}</a>
                            <ul>
                            ...
                            @if (EcommerceHelper::isCartEnabled())
                                ...
                                @else
                                    @if ($product->isOutOfStock())
                                        <button type="button" name="add_to_cart" value="1"
                                            class="btn btn-primary ms-2 add-to-cart-button disabled" disabled
                                            title="{{ __('Add to cart') }}">
                                            <span class="svg-icon">
                                                <svg>
                                                    <use href="#svg-icon-cart" xlink:href="#svg-icon-cart"></use>
                                ...
                                @if (EcommerceHelper::isQuickBuyButtonEnabled())
                                    @if (auth('customer')->check())
                                        <button type="button" name="checkout" value="1"
                                    ...
                                    @else
                                        @if ($product->isOutOfStock())
                                            <button type="button"
                                                class="btn btn-primary btn-black ms-2 add-to-cart-button" disabled
                                                title="{{ __('Buy Now') }}">
                                                <span class="add-to-cart-text">{{ __('Buy Now') }}</span>
                                            </button>
                                        @else
