...
use Botble\Ecommerce\Supports\CartHelper;
use Botble\Ecommerce\Supports\NotificationHelper;
    ...
    'events' => [
        ...
        'beforeRenderTheme' => function (Theme $theme) {
            ...
            if (is_plugin_active('ecommerce')) {
                ...
                $cart = $cartHelper->getAllDataCart();

                $notificationHelper = new NotificationHelper();
                $notification = $notificationHelper->getAllDataNotification();

                $theme->partialComposer('header', function ($view) use ($categories,$cart,$notification) {
                ...
                $theme->partialComposer('footer', function ($view) use ($categories,$cart,$notification) {
                    $view->with('categories', $categories);
                    $view->with('cart', $cart);
                    $view->with('notification', $notification);
                });

                $theme->partialComposer('cart-mini.list', function ($view) use ($cart,$notification) {
                    $view->with('cart', $cart);
                    $view->with('notification', $notification);
                });

                $theme->composer('ecommerce.includes.filters', function ($view) use ($categories,$cart,$notification) {
                    $view->with(
                        [
