...
app()->booted(function () {
    ...
    if (is_plugin_active('simple-slider') && is_plugin_active('ecommerce')) {
        // $shortcodes = '';
        // add_shortcode(
        ...
        // );
        add_filter(SIMPLE_SLIDER_VIEW_TEMPLATE, function () {
            return Theme::getThemeNamespace() . '::partials.shortcodes.slider-categories';
        }, 120);
    ...
    if (is_plugin_active('ecommerce')) {
        ...
        add_shortcode(
            ...
            __('Product category products'),
            function ($shortcode) {
                $category_id = explode(',',$shortcode->category_id);
                ...
                $category = [];
                $get_category_lev_1 = \DB::select("SELECT * FROM ec_product_categories1 WHERE id = '$category_id[0]' AND status = '".BaseStatusEnum::PUBLISHED."'");
                if(count($get_category_lev_1) > 0){
                    if(count($category_id) > 1){
                        $get_category_lev_2 = \DB::select("SELECT * FROM ec_product_categories2 WHERE id = '$category_id[1]' AND status = '".BaseStatusEnum::PUBLISHED."'");
                        if(count($get_category_lev_2) > 0){
                            if(count($category_id) > 2){
                                $get_category_lev_3 = \DB::select("SELECT * FROM ec_product_categories3 WHERE id = '$category_id[2]' AND status = '".BaseStatusEnum::PUBLISHED."'");
                                if(count($get_category_lev_3) > 0){
                                    $category = (object)$get_category_lev_3[0];
                                    $category->category_type = '3';

                                    $get_slug = \DB::select("SELECT * FROM slugs WHERE reference_id = '$category_id[2]' AND prefix = 'product-categories'");
                                    if(count($get_slug) > 0){
                                        $category->url = route('public.product-category',$get_slug[0]->key);
                                    }else{
                                        $category->url = '';
                                    }
                                }else{
                                    $category = (object)$get_category_lev_2[0];
                                    $category->category_type = '2';

                                    $get_slug = \DB::select("SELECT * FROM slugs WHERE reference_id = '$category_id[1]' AND prefix = 'product-categories'");
                                    if(count($get_slug) > 0){
                                        $category->url = route('public.product-category',$get_slug[0]->key);
                                    }else{
                                        $category->url = '';
                                ...
                                $get_slug = \DB::select("SELECT * FROM slugs WHERE reference_id = '$category_id[0]' AND prefix = 'product-categories'");
                                if(count($get_slug) > 0){
                                    $category->url = route('public.product-category',$get_slug[0]->key);
                                }else{
                                    $category->url = '';
                            ...
                            $category = (object)$get_category_lev_1[0];
        ...
        shortcode()->setAdminConfig('product-category-products', function ($attributes) {
            $categories = ProductCategoryHelper::getProductCategoriesWithIndentAdmin();
