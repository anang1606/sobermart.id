...
class DashboardController
{
    ...
    public function index(Request $request, BaseHttpResponse $response)
    {
		...
		$storelist = Store::where('customer_id', $user->id)->first();	
		$produklist = Product::where('store_id', $storelist->customer_id)->get();
		$jmlProducts = $produklist->count();	
		
		$orderlist = Order::where('store_id', $storelist->customer_id)->get();
		$jmlOrder = $orderlist->count();	
		
			
		$totalProducts = $store->products()->count();
        $totalOrders = $store->orders()->count();
        $compact = compact('user', 'store', 'data', 'totalProducts', 'totalOrders','jmlProducts','jmlOrder');
        if ($request->ajax()) {
            return $response
                ->setData([
