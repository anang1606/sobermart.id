    ...
    <div class="flexbox-layout-sections" id="main-order-content" style="margin: 0 -20px;">
                                    ...
                                    @foreach ($order->products as $orderProduct)
                                        @php
                                            $product = get_products([
                                                ...
                                                'take' => 1,
                                                'select' => ['ec_products.id', 'ec_products.images', 'ec_products.name', 'ec_products.price', 'ec_products.sale_price', 'ec_products.sale_type', 'ec_products.start_date', 'ec_products.end_date', 'ec_products.sku', 'ec_products.is_variation', 'ec_products.is_variation', 'ec_products.status', 'ec_products.order', 'ec_products.created_at'],
                                            ]);
                                        @endphp
                                                    ...
                                                    <ul class="unstyled">
                                                        <li class="simple-note">
                                                            <a><span>{{ $orderProduct->qty }}</span><span
                                                                    ...
                                                                    {{ trans('plugins/ecommerce::order.completed') }}</span></a>
                                                            <ul class="dom-switch-target line-item-properties small">
                                                                <li class="ws-nm">
                                                                    ...
                                                                    <strong>{{ $order->shipment->shipping_company_name }}</strong>
                                                                </li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                            ...
                            <div class="flexbox-auto-right pl5">
                                ...
                                <div class="d-flex w-100 justify-content-end">
                                    @if ($order->shipment->shipment_id)
                                            <a href="{{ route('marketplace.vendor.orders.generate-resi', $order->id) }}"
                                                class="btn btn-info me-3" target="_blank" rel="noopener noreferrer">
                        ...
                        <div class="next-card-section">
                            ...
                            <div>
                                @php
                                    $isVendor = true;
                                @endphp
                                @include('plugins/ecommerce::themes.includes.tracking-shipment', compact('order','isVendor'))
                            </div>
                        </div>
                    @endif
        ...
        @if ($order->status != \Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
            {!! Form::modalAction(
                ...
                trans('plugins/ecommerce::order.resend_order_confirmation'),
                'info',
                ...
                trans('plugins/ecommerce::order.send'),
            ) !!}
            {!! Form::modalAction(
                'update-shipping-address-modal',
                trans('plugins/ecommerce::order.update_address'),
                'info',
                view('plugins/ecommerce::orders.shipping-address.form', [
                    'address' => $order->address,
                    'orderId' => $order->id,
            ...
            {!! Form::modalAction(
                'cancel-order-modal',
                trans('plugins/ecommerce::order.cancel_order_confirmation'),
                'info',
                trans('plugins/ecommerce::order.cancel_order_confirmation_description'),
                'confirm-cancel-order-button',
                trans('plugins/ecommerce::order.cancel_order'),
            ) !!}
            ...
            @if ($order->shipment && $order->shipment->id !== '')
                {!! Form::modalAction(
                    ...
                    'info',
                    view('plugins/marketplace::themes.dashboard.orders.shipping-status-modal', [
                        'shipment' => $order->shipment,
