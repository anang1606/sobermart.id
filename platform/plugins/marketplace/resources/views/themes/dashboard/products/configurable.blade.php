<div id="product-variations-wrapper">
    <div class="variation-actions">
        <a href="#" class="btn-trigger-select-product-attributes"
            data-target="{{ route('marketplace.vendor.products.store-related-attributes', $product->id) }}">{{ trans('plugins/ecommerce::products.edit_attribute') }}</a>
        <a href="#" class="btn-trigger-generate-all-versions"
            data-target="{{ route('marketplace.vendor.products.generate-all-versions', $product->id) }}">{{ trans('plugins/ecommerce::products.generate_all_variations') }}</a>
    </div>
                    ...
                    <tr id="variation-id-{{ $variation->id }}">
                        <td>
                            <div class="wrap-img-product">
                                <img src="{{ RvMedia::getImageUrl($currentRelatedProduct && $currentRelatedProduct->image ? $currentRelatedProduct->image : $product->image, 'thumb', false, RvMedia::getDefaultImage()) }}"
                        ...
                        <td>
                            <label>
                                <input type="radio" {{ $variation->is_default ? 'checked' : '' }}
                                    name="variation_default_id" value="{{ $variation->id }}">
                            </label>
                        </td>
                        ...
                        <td style="width: 180px;" class="text-center">
                            <a href="#" class="btn btn-info btn-trigger-edit-product-version"
                                ...
                                data-load-form="{{ route('marketplace.vendor.products.get-version-form', [$variation->id]) }}">{{ trans('plugins/ecommerce::products.edit_variation_item') }}</a>
                            <a href="#"
                                data-target="{{ route('marketplace.vendor.products.delete-version', [$variation->id]) }}"
    ...
    <a href="#" class="btn-trigger-add-new-product-variation"
        ...
        data-processing="{{ trans('plugins/ecommerce::products.processing') }}">{{ trans('plugins/ecommerce::products.add_new_variation') }}</a>

    {!! Form::modalAction(
        'select-attribute-sets-modal',
        trans('plugins/ecommerce::products.select_attribute'),
        'info',
    ...
    {!! Form::modalAction(
        ...
        trans('plugins/ecommerce::products.edit_variation'),
        'info',
        view('core/base::elements.loading')->render(),
    ...
    {!! Form::modalAction(
        ...
        trans('plugins/ecommerce::products.delete_variation'),
        'danger',
