...
namespace Botble\AdsCategory\Tables;
...
class AdsCategoryTable extends TableAbstract
{
    ...
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, AdsCategoryRepository $adsCategoryRepository)
    {
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->hasPermission('category-ads.edit')) {
                    return BaseHelper::clean($item->name);
                }

                return Html::link(route('category-ads.edit', $item->id), BaseHelper::clean($item->name));
            })
            ...
            ->editColumn('categories_id', function ($item) {
                if($item->categories_id === 'all'){
                    return 'Semua Kategori';
                }else{
                    $getCategory = ProductCategory::find($item->categories_id);
        ...
        $data = $data->addColumn('operations', function ($item) {
            return $this->getOperations('category-ads.edit', 'category-ads.destroy', $item);
        });
    ...
    {
        $buttons = $this->addCreateButton(route('category-ads.create'), 'category-ads.create');
    ...
    {
        return $this->addDeleteAction(route('category-ads.deletes'), 'category-ads.destroy', parent::bulkActions());
    }
