...
@section('content')
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end p-r5">
                                                    {{ format_price($order->amount + $order->shipping_amount - $order->discount_amount) }}
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                    ...
                    <div class="pd-all-10-20">
                        <form action="{{ route('orders.edit', $order->id) }}">
                            ...
                            <textarea class="ui-text-area textarea-auto-height" name="description"
                                placeholder="{{ trans('plugins/ecommerce::order.order_note_placeholder') }}" rows="2">{{ $order->description }}</textarea>
                            ...
                            <li class="overflow-ellipsis">
                                <div class="mb5">
                                    <a class="hover-underline text-capitalize"
                                        href="#">{{ $order->user->name ?: $order->address->name }}</a>
                                </div>
                                @if ($order->user->id)
                                    <div><i class="fas fa-inbox mr5"></i><span>{{ $order->user->orders()->count() }}</span>
                                ...
                                <div>
                                    ...
                                    <div>
                                        <a target="_blank" class="hover-underline"
                                            href="https://maps.google.com/?q={{ $order->full_address }}">{{ trans('plugins/ecommerce::order.see_maps') }}</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
    ...
    {!! Form::modalAction(
        ...
        trans('plugins/ecommerce::order.notice_about_incomplete_order'),
        'info',
