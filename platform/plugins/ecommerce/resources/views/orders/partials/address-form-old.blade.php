<div class="customer-address-payment-form">
                                ...
                                <option value="{{ $address->id }}"
                                    @if (
                                        ($address->is_default && !$sessionAddressId) ||
                                            ...
                                            (!old('address.address_id', $sessionAddressId) && $loop->first)) selected="selected" @endif>
    ...
    <div class="address-form-wrapper @if (auth('customer')->check() &&
            $isAvailableAddress &&
            ((!empty($sessionAddressId) && $sessionAddressId !== 'new') || empty(Arr::get($sessionCheckoutData, 'state')))) d-none @endif">
        <div class="row">
            <div class="col-12">
        ...
        <div class="row">
            <div class="col-lg-8 col-12">
                <div class="form-group  @if ($errors->has('address.email')) has-error @endif">
                    ...
                    <input type="text" name="address[email]" id="address_email" placeholder="{{ __('Email') }}"
                        class="form-control address-control-item address-control-item-required checkout-input"
                    ...
                    <div class="form-group mb-3 @if ($errors->has('address.country')) has-error @endif">
                        <div class="select--arrow">
                            <select name="address[country]"
                                ...
                                id="address_country" data-type="country">
                    ...
                    @if (EcommerceHelper::loadCountriesStatesCitiesFromPluginLocation())
                        <div class="select--arrow">
                            <select name="address[state]"
                                class="form-control address-control-item address-control-item-required"
                                id="address_state" data-type="state" data-url="{{ route('ajax.states-by-country') }}">
    ...
    @if (!auth('customer')->check())
        <div class="form-group mb-3">
            <input type="checkbox" name="create_account" value="1" id="create_account"
                @if (empty($errors) && old('create_account') == 1) checked @endif>
            <label for="create_account" class="control-label"
                style="padding-left: 5px">{{ __('Register an account with above information?') }}</label>
        </div>
