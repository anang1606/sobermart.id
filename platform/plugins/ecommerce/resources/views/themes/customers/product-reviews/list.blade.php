    ...
    <div class="section-content product-reviews-page">
        <ul class="nav nav-tabs nav-fill" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link @if (!request()->has('page')) active @endif" id="waiting-tab" data-toggle="tab"
                    data-target="#waiting-tab-pane" data-bs-toggle="tab" data-bs-target="#waiting-tab-pane" type="button"
                    role="tab" aria-controls="waiting-tab-pane" aria-selected="true">{{ __('Waiting for your review') }}
                    ({{ $products->count() }})</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link @if (request()->has('page')) active @endif" id="reviewed-tab" data-toggle="tab"
                    data-target="#reviewed-tab-pane" data-bs-toggle="tab" data-bs-target="#reviewed-tab-pane" type="button"
                    role="tab" aria-controls="reviewed-tab-pane" aria-selected="false">{{ __('Reviewed') }}
                    ({{ $reviews->total() }})</button>
            </li>
        </ul>
        ...
        <div class="tab-content border border-top-0 p-2">
                                                ...
                                                @for ($i = 5; $i >= 1; $i--)
                                                    <label class="order-{{ $i }}">
                                                        <span class="ecommerce-icon" data-star="{{ $i }}">
                                                            <svg>
                                                                <use href="#ecommerce-icon-star"
                                                                    xlink:href="#ecommerce-icon-star"></use>
                                                            </svg>
                                                        </span>
                                                    </label>
                                                @endfor
            ...
            <div class="tab-pane fade show active @if (request()->has('page')) show active @endif"
                id="reviewed-tab-pane" role="tabpanel" aria-labelledby="reviewed-tab" tabindex="0">
