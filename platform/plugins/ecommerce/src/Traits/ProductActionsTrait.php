...
trait ProductActionsTrait
{
    ...
    public function postSaveAllVersions(
        ...
        foreach ($versionInRequest as $variationId => $version) {
            ...
            if (!$variation->product_id || $isUpdateProduct) {
                ...
                $images = json_encode(array_values(array_filter((array)Arr::get($version, 'images', []) ?: [])));
                ...
                $productRelatedToVariation->etalase = $product->etalase;
                $productRelatedToVariation->description = $product->description;
                $productRelatedToVariation->content = $product->content;
                $productRelatedToVariation->is_variation = 1;

                $productRelatedToVariation->kategori1 = $product->kategori1;
                $productRelatedToVariation->kategori2 = $product->kategori2;
                $productRelatedToVariation->kategori3 = $product->kategori3;
    ...
    public function postUpdateVersion(
        ...
        if (!empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            return $this->postSaveAllVersions(
