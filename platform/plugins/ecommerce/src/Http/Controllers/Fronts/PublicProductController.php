...
class PublicProductController
{
    ...
    public function getProduct($slug, Request $request)
    {
        ...
        if (count($selectedAttrs) > 0) {
            foreach ($selectedAttrs as $selAttr) {
                $product_title .= ' - ' . $selAttr->title;
                if($selAttr->is_default){
                    $is_default_variant = $selAttr->id;
                }
            }
        }

        $product->with_storehouse_management = $productVariation->with_storehouse_management;
        if($product->with_storehouse_management){
            $product->quantity = $productVariation->quantity;
        }
        $product->title = $product_title;
        // return $product;
        $flash_sale_status = true;
        $get_flash_sale = \DB::select("SELECT * FROM ec_flash_sale_products WHERE product_id = '".$product->id."'");
        if(count($get_flash_sale) > 0){
            $stock_sale = (int)$get_flash_sale[0]->quantity - (int)$get_flash_sale[0]->sold;
            if((int)$stock_sale === 0){
                $flash_sale_status = false;
            }
        }
        $product->flash_sale_status = $flash_sale_status;

        $variant_id = [];
        [$productImages,$allProductVariant,$selectedAttrs] = EcommerceHelper::getProductVariationAll($product);


        if(is_array($allProductVariant)){
            if(count($allProductVariant) > 0){
                foreach($allProductVariant as $allVariant){
                    $variant_id[] = $allVariant->id;
                }
            }else{
        ...
        // $terjual = $product->original_product->terjual;
        // return $variant_id;

        // return $product->original_product;
    ...
    public function getProductCategory(
        ...
        if((int)$category->parent_id !== 0){
            $getParentCategory = ProductCategory::find($category->parent_id);
            if($getParentCategory){
                $parent_id = $getParentCategory->id;
            }
        }else{
        ...
        return Theme::scope(
            ...
            compact('category', 'products','getAds'),
    ...
    public function getProductVariation($id, Request $request, BaseHttpResponse $response)
    {
        ...
        $product_title = $product->name;
        $variation_id = $this->productVariationRepository->getAttributeIdsOfChildrenProduct($product->id);
        if (count($variation_id) > 0) {
            foreach ($variation_id as $varId) {
                $get_variation = \DB::select("SELECT * FROM ec_product_attributes WHERE id = '$varId'")[0];
                if ($get_variation) {
                    $product_title .= ' - ' . $get_variation->title;
                }
            }
        }
    ...
    protected function ajaxFilterProductsResponse($products, Request $request, BaseHttpResponse $response, ?ProductCategory $category = null)
    {
        ...
        if($currentRouteName === "public.products"){
            $parent_id = 'all';
        }
        $getAds = AdsCategory::where([
            ['categories_id',$parent_id]
        ...
        $viewAds = Theme::getThemeNamespace('views.ecommerce.includes.ads-categories');

        return $response
            ->setData(array(
                'view' => view($view, compact('products'))->render(),
                'ads' => view($viewAds,compact('getAds'))->render(),
            ))
