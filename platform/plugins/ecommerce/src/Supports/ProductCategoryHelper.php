...
class ProductCategoryHelper
{
    ...
    public function getAllProductCategoriesAdmin(array $params = [], bool $onlyParent = false): Collection
    {
        if (! $this->allCategories instanceof Collection) {
            $this->allCategories = collect();
        }
        ...
        if ($this->allCategories->count() == 0) {
            $query = DB::table('ec_product_categories1');

            // Kondisi
            $condition = Arr::get($params, 'condition', []);
            ...
            //         // Tambahkan logika join sesuai dengan kebutuhan
            //         // Contoh: $join->on('product_categories.id', '=', $relation.'.category_id');
            //     });
            // }
    ...
    public function getAllProductCategoriesSortByChildrenAdmin(): Collection
    {
        // $categories = $this->getAllProductCategories();
        $categories = $this->getAllProductCategoriesAdmin();
    ...
    public function getProductCategoriesWithIndentAdmin(string $indent = '&nbsp;&nbsp;', bool $sortChildren = true): Collection
    {
        $categories = $this->getAllProductCategoriesSortByChildrenAdmin();
