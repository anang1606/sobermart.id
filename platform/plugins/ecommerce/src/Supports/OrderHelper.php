...
class OrderHelper
{
    public function processOrderTesting(string|array|null $orderIds, ?string $chargeId = null)
    {
        DB::beginTransaction();
        try {
    ...
    public function processOrder(string|array|null $orderIds, $chargeId = null): bool|Collection|array|Model
    {
        $orderIds = (array)$orderIds;
        $orders = app(OrderInterface::class)->allBy([['id', 'IN', $orderIds]]);
    ...
    public function handleAddCart(Product $product, Request $request,bool $isBuynow = false)
    {
        ...
        if ($request->input('options')) {
            ...
            $options_request = $request->input('options');
            foreach ($options_request as $key => $option) {
                $optionValue = OptionValue::select(['option_id','id'])->where('option_id', $key);
                ...
                $options[] = $optionValue->first();
            }
            $options_cart = implode(';',$options);
        }
        ...
        if($isBuynow){
            if($product->is_variation){
                ...
                if($options_cart){
                    $get_buynow = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',1],['attributes',$attributes->id],['options',$options_cart]])->first();
                }else{
                    $get_buynow = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',1],['attributes',$attributes->id]])->first();
                }
            }else{
                $get_buynow = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',1]])->first();
            }
            ...
            $new_cart->qty = $request->input('qty', 1);
            $new_cart->options = $options_cart;
            $new_cart->is_buynow = ($isBuynow) ? 1 : 0;
            $new_cart->extras = implode(',', $request->input('extras', []));
            $new_cart->attributes = $product->is_variation ? ProductVariation::where('product_id', $product->id)->first()->id : '';

            if($new_cart->save()){
                session()->put('checkout_selected_cart',$new_cart->id);
                return $new_cart;
            }
        }
        ...
        if($product->is_variation){
            $attributes = ProductVariation::where('product_id', $product->id)->first();
            if($options_cart){
                $get_cart = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',0],['attributes',$attributes->id],['options',$options_cart]])->first();
            }else{
                $get_cart = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',0],['attributes',$attributes->id]])->first();
            }
        }else if($options_cart){
            $get_cart = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',0],['options',$options_cart]])->first();
        }else{
            $get_cart = CartModel::where([['customer_id', $customer_id],['product_id', $product->id],['is_buynow',0]])->first();
        }

        if ($get_cart) {
            $get_cart->qty = (int)$get_cart->qty + (int)$request->input('qty', 1);
            $get_cart->save();
        } else {
            $new_cart = new CartModel;
            $new_cart->customer_id = $customer_id;
        ...
        foreach (CartModel::where([['customer_id', $customer_id],['is_buynow',0]])->get() as $item) {
            $cartItems[] = $item;
        }
