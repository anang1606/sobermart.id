...
class EcommerceHelper
{
    ...
    public function getProductReviews(Product $product, int $star = 0, int $perPage = 10): LengthAwarePaginator
    {
        ...
        if ($star && $star >= 1 && $star <= 5) {
            ...
            $condition['ec_reviews.parent_id'] = Null;
        }
    ...
    public function getProductVariationInfo(Product $product): array
    {
        ...
        if ($product->variations()->count()) {
            if ($product->is_variation) {
                ...
                // $selectedAttrs[] = ProductVariation::where(
                //     'product_id',
                //     $product->id
    ...
    public function getProductVariationAll(Product $product): array
    {
        $productImages = $product->images;

        $allProductVariant = $product;
        ...
        if ($product->variations()->count()) {
            ...
            if ($variationDefault) {
                $allProductVariant = app(ProductInterface::class)->getProductVariations($product->id, [
                    'condition' => [
                        // 'ec_product_variations.id' => $variationDefault->id,
                        'original_products.status' => BaseStatusEnum::PUBLISHED,
                    ],
        ...
        return [$productImages, $allProductVariant, $selectedAttrs];
    }
